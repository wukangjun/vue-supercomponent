    ...
    <div class="main">
        ...
        <div class="right">
            <el-row style="height:100%;">
                <component :is="$store.getters.mapName" style="width: 100%;height: 100%;overflow: hidden;margin:0;" id="fenceMainZT"></component>
            </el-row>
        </div>
...
export default {
    ...
    methods: {
        ...
        searchClickLoadGrid(flag) {
            ...
            this.LoadGrid(this.fence.fenceName, this.fence.fenceType,"103");
        },
        ...
        selectFence(data) {
            if (this.overlay)
                this.$map["fenceMainZT"].removeOverlay(this.overlay);
            if (this.infoWindow)
                this.$map["fenceMainZT"].closeInfoWindow(this.infoWindow);
            this.grid.loading = true;
            let params = { id: data.row.id, mapType: 1, type: data.row.type };
                ...
                if (data.flag) {
                    ...
                    switch (data.data.ruleType) {
                        ...
                        case 1:
                            ...
                            this.overlay = this.$map["fenceMainZT"].mapAddcircle(points, radius);
                            content = this.tContent(data.data);
                            // console.log(this.$map["fenceMainZT"])
                            point = this.$map["fenceMainZT"].point(points);
                            this.$map["fenceMainZT"].overlayClickOpenInfoWindow(this.overlay, point, content, callbackInfoWindow, {});
                            break;
                        //多边形
                        case 3:
                            let polygonPoints = [];
                            if (data.data.requestPoint.length > 0) {
                                ...
                                this.overlay = this.$map["fenceMainZT"].mapAddpolygon(polygonPoints, {});
                                content = this.tContent(data.data);
                                point = this.$map["fenceMainZT"].point(polygonPoints[0]);
                                this.$map['fenceMainZT'].getBestView(polygonPoints);
                                this.$map['fenceMainZT'].openInfoWindow(content,this.overlay,this.overlay.getBounds().getCenter())
                                this.infoWindow = this.$map['fenceMainZT'].infoWindow(content)
                                console.log(this.infoWindow)
                                this.$map["fenceMainZT"].overlayClickOpenInfoWindow(this.overlay, this.overlay.getBounds().getCenter(), content, callbackInfoWindow, {});    
                            }
                            break;
                        //折线
                        case 4:
                            if (data.data.requestPoint.length > 0) {
                                ...
                                this.overlay = this.$map["fenceMainZT"].mapAddline(polylinPoints, {});
                                content = this.tContent(data.data);
                                point = this.$map["fenceMainZT"].point(polylinPoints[0]);
                                this.$map['fenceMainZT'].getBestView(polylinPoints);
                                this.$map["fenceMainZT"].overlayClickOpenInfoWindow(this.overlay, point, content, callbackInfoWindow, {});
                            }
                            break;
                        default:
                ...
                for (let i = 0; i < areaArr.length; i++) {
                    switch (data.areaNameAndType[i].areaType) {
                        case 2:
                            ...
                            content += `<div style='text-indent:40px;font-size:12px;'><span>` + this.$t("fenceManage.limitrpm") + `：</span><span>${areaArr[i].limitrpm==null?'--':areaArr[i].limitrpm} (r/min)</span></div>`;
                            content += `<div style='text-indent:40px;font-size:12px;'><span>` + this.$t("fenceManage.aspeed") + `：</span><span>${areaArr[i].aspeed==null?'--':areaArr[i].aspeed} (km/h)</span></div>`;
                            break;
                        case 256:
                            content += `<div style='text-indent:20px;font-size:14px;'><span>${areaArr[i].areaName} :</span></div>`;
                            content += `<div style='text-indent:40px;font-size:12px;'><span>` + this.$t("fenceManage.limitrpm") + `：</span><span>${areaArr[i].limitrpm} (r/min)</span></div>`;
        ...
        addFence() {
                this.options.name = "fenceActionzt";
                this.options.id = "";
                this.options.isModify = false;
                this.options.type = 103
        },
        updateFence(val) {
                this.options.name = "fenceActionzt";
                ...
                this.$http({
                    ...
                    if (data.flag) {
                        ...
                        if (this.overlay)
                            this.$map["fenceMainZT"].removeOverlay(this.overlay);
                            // console.log(this.infoWindow)
                            this.$map["fenceMainZT"].clearOverlays()
                    } else {
                        this.$message.error({ message: this.$t("error." + data.errorCode) });
                    }
                });
