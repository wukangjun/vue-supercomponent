    ...
    <div class="main">
        ...
        <div class="right">
            <el-row style="height:100%;">
                <component :is="$store.getters.mapName" style="width: 100%;height: 100%;overflow: hidden;margin:0;" id="fenceMain"></component>
            </el-row>
        </div>
...
export default {
    ...
    methods: {
        ...
        selectFence(data) {
            if (this.overlay)
                this.$map["fenceMain"].removeOverlay(this.overlay);
            if (this.infoWindow)
                this.$map["fenceMain"].closeInfoWindow(this.infoWindow);
            this.grid.loading = true;
                ...
                if (data.flag) {
                    ...
                    switch (data.data.ruleType) {
                        ...
                        case 1:
                            ...
                            this.overlay = this.$map["fenceMain"].mapAddcircle(points, radius);
                            content = this.tContent(data.data);
                            point = this.$map["fenceMain"].point(points);
                            this.$map["fenceMain"].overlayClickOpenInfoWindow(this.overlay, point, content, callbackInfoWindow, {});
                            this.$map["fenceMain"].openInfoWindow(content, this.overlay, point);
                            break;
                        //矩形
                        case 2:
                            let rectangle = [];
                            ...
                            if(data.data.requestPoint.length>0){
                                ...
                                this.overlay = this.$map["fenceMain"].mapAddrectangle(rectangleData, {});
                                content = this.tContent(data.data);
                                point = this.$map["fenceMain"].point(rectangle[0]);
                                this.$map['fenceMain'].getBestView(rectangle);
                                this.$map["fenceMain"].overlayClickOpenInfoWindow(this.overlay, this.overlay.getBounds().getCenter(), content, callbackInfoWindow, {});
                                this.$map["fenceMain"].openInfoWindow(content, this.overlay, this.overlay.getBounds().getCenter());
                            };
                            break;
                        //多边形
                        case 3:
                            ...
                            if (data.data.requestPoint.length > 0) {
                                ...
                                this.overlay = this.$map["fenceMain"].mapAddpolygon(polygonPoints, {});
                                content = this.tContent(data.data);
                                point = this.$map["fenceMain"].point(polygonPoints[0]);
                                this.$map['fenceMain'].getBestView(polygonPoints);
                                this.$map["fenceMain"].overlayClickOpenInfoWindow(this.overlay, this.overlay.getBounds().getCenter(), content, callbackInfoWindow, {});
                                this.$map["fenceMain"].openInfoWindow(content, this.overlay, this.overlay.getBounds().getCenter());
                            }
                            break;
                        ...
                        case 4:
                            if (data.data.requestPoint.length > 0) {
                                ...
                                this.overlay = this.$map["fenceMain"].mapAddline(polylinPoints, {});
                                content = this.tContent(data.data);
                                point = this.$map["fenceMain"].point(polylinPoints[0]);
                                this.$map['fenceMain'].getBestView(polylinPoints);
                                this.$map["fenceMain"].overlayClickOpenInfoWindow(this.overlay, point, content, callbackInfoWindow, {});
                                this.$map["fenceMain"].openInfoWindow(content, this.overlay, point);
                            }
                            break;
            ...
            this.$confirm(this.$t("action.confirmIsRemove"), this.$t("action.tip"), {
                ...
                this.$http({
                    ...
                    if (data.flag) {
                        ...
                        if (this.overlay)
                            this.$map["fenceMain"].removeOverlay(this.overlay);
                            this.$map["fenceMain"].clearOverlays()
                    } else {
                        this.$message.error({ message: this.$t("error." + data.errorCode) });
                    }
                });
