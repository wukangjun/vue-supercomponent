<template>
	<div class="multiCar" :style="{top: isFull?'0px':'90px'}">
		...
		<baidu-map id="monitorTrailMap" class="multiMap" @ready="mapReady"></baidu-map>
		...
		<div class="tool">
			<span class="toolItem" v-show='! this.$store.getters.isMonitorOrTrack'>
				跟踪: <el-switch v-model="isMulti"></el-switch>
			</span>
			<span class="toolItem">
		...
		methods: {		
			...
			async addEvent(array){
				...
				if(this.$service.isArray(data)){
					this.$service.chunk(data, (val, len) => {
						...
						arr.length === 1 ? this.$map['monitorTrailMap'].moveToCenter(arr[arr.length-1], 17) :
						this.$map['monitorTrailMap'].getBestView(arr);		
						//this.loopCar();
					}, 30)
				}else{	
					this.mapFlag = false;
			...
			removeEvent(array){
				array.forEach(ele => {
					...
					this.$map['monitorTrailMap'].removeOverlay(marker);
			...
			pointMap({lng, lat, vehicleId, data}){
				if(data){
					let marker = this.$map['monitorTrailMap'].mapAddpoint({
						longitude: Number(lng), 
						latitude: Number(lat)
					}, this.$filter.isOnline(data), {width: 32,height: 32});
					/* 给地图添加点和文字 */
					marker.label=this.$map['monitorTrailMap'].addLabel(marker, data.plateCode);
					this.$map['monitorTrailMap'].addOverlay(marker);	
			...
			moveMarker: function(marker, data){
				if(marker && data){
					let img = this.$filter.isOnline(data),
						icon = this.$map['monitorTrailMap'].setIcon(img, 32, 32),
						point = this.$map['monitorTrailMap'].point({lng: data.mapLongitude, lat: data.mapLatitude});
					marker.setIcon(icon);
					...
					if(this.$service.isArray(data)){
						this.$service.chunk(data, val => {
							...
							if(this.$store.getters.isGetBaseView && baseMarker){
								this.$map["monitorTrailMap"].mapPanTo(baseMarker.point);
							}
							// 更新完最后一个的回调		
			...
			mapBaseEvent(){				
				...
				if(!this.$store.getters.isMonitorOrTrack){
					...
					this.$map['monitorTrailMap'].getBestView(markArry);
				}							
			},
			// 关闭定时器
			closeTimeout(){
