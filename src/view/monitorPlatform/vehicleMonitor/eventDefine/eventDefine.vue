<template>
<div class="eventDefine">
	<cv-header>
		<el-form-item style="float:right">
			<cv-tool style="margin: 0px" :name="$route.name" :data="$store.getters.monitorMenu" :is-modify="false" :is-remove="true" :is-export="false" @add="add" @remove="removeBatch" @search="search"></cv-tool>
		</el-form-item>
	</cv-header>
	<cv-content :fill='true'>
		<cv-grid :options="grid.options" @load="load(false)">
			<el-table border stripe :data="grid.table" v-loading="grid.loading" ref="table" @selection-change="handleSelectionChange">
				...
				<el-table-column :label="$t('eventInfo.eventinfo')" prop="eventinfo" :show-overflow-tooltip="true"></el-table-column>
  ...
  methods: {
    ...
    eventInfoList() {
      let params = {
        ...
        eventinfo: this.dialog.search.eventinfo
      };
      this.grid.loading = true;
      this.$store
        .dispatch("getEventInfoPageEventDefine", params)
        .then(data => {
    ...
    remove(scope) {
      this.confirmPrompt().then(() => {
        this.$store.dispatch("deleteEventInfoById", scope.row.id).then(data => {
          // console.log(data);
          if (!data.flag && !!data.errorCode) {
            var infoMsg=data.data.data?data.data.data.msg:"";
            this.$message.error({
              message: infoMsg+this.$t("error." + data.errorCode)
            });
            //this.$notify.error({message: this.$t("error."+data.errorCode)});
          } else {
            this.load();
            this.$message.success({
              message: this.$t("action.removeSuccess")
            });
    ...
    removeBatch() {
      if (this.getSelectedCount() > 0) {
        this.confirmPrompt().then(() => {
          this.$store
            .dispatch("deleteEventInfoByBatch", this.getSelectedIds().join(","))
            .then(data => {
              // console.log(data);
              if (!data.flag && !!data.errorCode) {
                var infoMsg=data.data.data?data.data.data.msg:"";
        ...
        this.$message.warning({
          message: this.$t("eventInfo.alertToSelect")
        });
      }
    },
    //打开修改弹框
