    ...
    <el-form :model="formData" label-position="top" :inline="false" :rules="rules" ref="addForm">
      ...
      <el-col :span="12">
        <el-form-item label="809服务端口" prop="portStr">
          <el-input v-model.trim="formData.portStr" size="small" :maxlength='4'></el-input>
        </el-form-item>
      </el-col>
      <!-- 自动重连 -->
      <el-col :span="12">
        <el-form-item label="自动重连" prop="">
    ...
    confirm() {
      this.$refs["addForm"].validate(valid => {
        if (valid) {
          ...
          this.loading = true;
          this.$http({
              url: this.$COMMON.baseUrl + "/serviceManagement/insertEznService",
              params: params
            })
            .then(data => {
              this.loading = false;
