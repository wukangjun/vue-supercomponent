            ...
            verificationAccount(rules, value, callback) {
                if (!this.$service.isContext(value)) {
                    if (value != this.account) {
                        this.$store.dispatch('checkEntUsers',{ account: value }).then(data => {
                            if (!data.flag) {
            ...
            confirm(){
                ...
                _self.$refs['addForm'].validate(valid => {
					if (valid) {
                        ...
                        _self.$store.dispatch('insertVehicleMaster', _par).then(data => {
                            if (data.flag) {
                                ...
                                _self.options.insertVehicleMaster=true;
                                _self.$emit('confirm');
                            } else {
                                _self.options.insertVehicleMaster=false;
