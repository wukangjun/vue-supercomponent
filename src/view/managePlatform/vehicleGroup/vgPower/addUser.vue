    ...
    <div class="up-add">
        <el-row>
            ...
            <el-col :span="10" style="width:44.66667%">
                <el-form>
                    <el-form-item :label="options.enterpriseName + $t('vgPower.uall')">
                        <el-input
                        :placeholder="$t('vgPower.typeAccount')"
                        icon="search"
                        v-model.trim="gridL.account"
            ...
            <el-col :span="10" style="width:44.66667%">
                <el-form>
                    <el-form-item :label="options.vgName + $t('vgPower.u')">
                        <el-input
                        :placeholder="$t('vgPower.typeAccount')"
                        icon="search"
                        v-model.trim="gridR.account"
        ...
        methods:{
            ...
            listL(){
                let params = {
                    ...
                    account:this.gridL.account,
                    accountType:"2,3"
                }
                this.gridL.loading = true;
                this.getEntNoFleetUserList(params).then(data=>{
                    if(data.flag){
            ...
            listR(){
                let params = {
                    ...
                    account:this.gridR.account,
                    enterpriseId:this.gridR.enterpriseId,
                    accountType:"2,3"
                }
            ...
            addClick(){
                if(this.gridL.multipleSelection.length !== 0){
                    let params = {
                        userIds:this.gridL.multipleSelection.join(","),
                        fleetId:this.gridL.fleetId,
                        enterpriseId:this.gridL.enterpriseId
                    }
                    this.loadFlag = true;
                    this.addFleetUser(params).then(data=>{
                        if(data.flag){
                    ...
                    this.$message.error(this.$t('vgPower.choseAccount'));
                }
            }
        },
        //监听弹框，每次弹框消失后重置弹框
