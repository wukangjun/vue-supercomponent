			...
			save(){
				...
				this.$refs['form'].validate(valid => {
					if(valid){
						...
						if(_self.form.channels.length==0){
							_self.$message.error('请先选中摄像头');	
							return false;
						}
						...
						for(let i = 0,len=this.form.channels.length;i<len;i++){
							...
							_self.cameraQuest(params).then(data=>{
								if(data.flag){
									...
									return _self.options.flag = false;
								}else{
						...
						// 	let _self = this;			
						// 	(function loop(i){
						// 		if(i >= _self.form.channels.length){
						// 			_self.submitFalg = false;
						// 			_self.options.flag = false;
						// 			// return _self.$message.success('指令下发成功');		
						// 		}	
						// 		params.channel = _self.form.channels[i];
						// 		_self.cameraQuest(params).then(data => {
						// 			if(data.flag){
						// 				 _self.$message.success('指令下发成功');
						// 				 _self.submitFalg = false;
