	...
	<el-form 
		...
		<el-form-item label="分辨率1:" prop="resolution">
			<el-select size="small" v-model="form.resolution">
	            ...
	            <el-option label="176*144" value="5"></el-option>
	            <el-option label="352*288" value="6"></el-option>
	            <el-option label="704*288" value="7"></el-option>
	            <el-option label="704*576" value="8"></el-option>
			</el-select>
		</el-form-item>
		...
		data(){
			return{
				...
				rules: {
					...
					lightness: {pattern: /^(\d{1}|[1-9]\d{1}|1\d{2}|2[0-4]\d{1}|25[0-5])$/, message:'0-255的数字', trigger: 'blur, change'},
					contrast: {pattern: /^(\d{1}|[1-9]\d{1}|1[0-2]{1}[0-7]{1})$/, message:'0-127的数字', trigger: 'blur, change'},
				...
				this.$refs['form'].validate(valid => {
					if(valid){
						let params = {
							...
							vehicleId: Number(this.options.id)
						};
						this.submitFalg = true;
						for(let i = 0,len=this.form.channels.length;i<len;i++){
							(async () => {
								params.channel = this.form.channels[i];
						...
						instructCallback(true, this.options);
					}else{
						return false
					}
				})
